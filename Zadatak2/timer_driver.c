...
static void setup_and_start_timer(unsigned int milliseconds, int direction);
...
static void setup_and_start_timer(unsigned int milliseconds, int direction)
{
	...
	if (!direction) 
		timer_load = milliseconds*100000; //broji od load ka nuli 
	else
	...
	if(ret == 2)//two parameters parsed in sscanf
	{
		...
		else
		{
			if (!strcmp(direction, "up"))
				...
				printk(KERN_INFO "xilaxitimer_write: wrong direction\n");
			
			printk(KERN_INFO "xilaxitimer_write: Starting timer for %d interrupts. One every %d miliseconds. Direction of counting is %s \n",number,millis,direction);
			i_num = number;
			setup_and_start_timer(millis,dir);
		}
